...
for(let i = 0;i<arrayData.length;i++){
    setValueProducts[i].children[1].children[0].children[0].src = arrayData[i].image;
    setValueProducts[i].children[2].children[0].innerHTML = `${arrayData[i].name}`;
    setValueProducts[i].children[3].children[0].innerHTML = `${arrayData[i].price}`
}
...
titleSet.addEventListener('click',function(){
    console.log('hihi')
    let x = JSON.stringify(arrayData);
...
function showMore(){
  ...
  if(clickbtn.innerHTML === 'Xem Them'){
    clickbtn.innerHTML = 'An Bot'
  }
  else {
    clickbtn.innerHTML = 'Xem Them';
  console.log('a');
  }
}
